...
export function MixSelector() {
  ...
  return (
    <div className="surface-panel overflow-hidden animate-rise-in-soft">
      {!showResult && (
        <div className="border-b border-border/70 bg-white/58 p-4 sm:p-5">
            ...
            <div className="grid grid-cols-3 gap-2">
              {steps.map(({ stage, label }) => (
                <div key={stage} className="flex min-w-0 flex-col items-center text-center">
                  ...
                  <span
                    className={`mt-3 text-[11px] font-medium sm:text-sm ${
      ...
      <div className="p-4 sm:p-5">
        ...
        {showResult && recommendation && (
          <div className="space-y-4 animate-rise-in-soft sm:space-y-5">
            <div className="glass-card rounded-2xl border border-primary/25 p-5 shadow-[0_16px_32px_-24px_hsl(var(--primary)/0.45)] sm:p-8">
              <div className="text-center">
                <div className="mb-3 text-sm font-medium text-muted-foreground">Рекомендуемый класс бетона</div>
                <div className="mb-2 text-4xl font-bold text-primary sm:text-6xl">{recommendation.recommendedClass.name}</div>
                <div className="font-mono text-xl text-foreground sm:text-2xl">{formatMpa(recommendation.recommendedClass.strengthMPa)}</div>
              </div>
            </div>
