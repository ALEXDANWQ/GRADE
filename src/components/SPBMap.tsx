...
export function SPBMap({ apiKey }: SPBMapProps) {
  ...
  return (
    <div className="surface-panel relative overflow-hidden animate-rise-in-soft">
      <div className="relative border-b border-border/70 bg-white/75 p-4 sm:p-5">
        ...
        {status === 'no-key' && (
          <div className="flex h-full items-center justify-center bg-secondary/30 p-6">
            <div className="glass-card max-w-xl rounded-2xl border border-amber-500/25 p-5">
              ...
              <pre className="mt-4 overflow-x-auto rounded-lg border border-border/70 bg-white/70 p-3 text-xs">
{`# .env.local
VITE_YMAPS_API_KEY=ваш_ключ_яндекс_карт`}
        ...
        {selectedBuilding && (
          <div className="pointer-events-none absolute inset-0 p-3 md:p-4">
            <div className="glass-overlay pointer-events-auto ml-auto flex w-full max-w-[430px] animate-fade-in-scale flex-col overflow-hidden rounded-2xl border border-border/75 shadow-[0_26px_54px_-34px_rgb(15_23_42/0.45)] sm:rounded-3xl">
