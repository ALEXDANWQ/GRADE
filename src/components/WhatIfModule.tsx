              ...
              {structures.map((structure) => {
                ...
                return (
                  <button
                    ...
                    className={`action-chip group rounded-2xl px-3 py-3 text-left ${
                      isSelected
                        ? 'border-foreground/26 bg-card text-foreground shadow-[inset_0_0_0_1px_hsl(var(--foreground)/0.18)]'
                        : 'border-border/75 bg-card/74 text-muted-foreground hover:text-foreground'
                    }`}
                  >
                    <span
                      className={`mb-2 inline-flex h-9 w-9 items-center justify-center rounded-lg border ${
                        isSelected
                          ...
                          : 'border-border/75 bg-card text-muted-foreground transition-colors duration-300 group-hover:border-foreground/20 group-hover:text-foreground'
                      }`}
                    >
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-7">
              {concreteClasses.map((cls) => (
                <button
                  ...
                  className={`action-chip rounded-xl px-3.5 py-2 text-center text-sm font-semibold ${
                    selectedClass.id === cls.id
                      ? 'border-black bg-black text-white hover:border-black hover:bg-black hover:text-white dark:border-white dark:bg-white dark:text-zinc-900 dark:hover:border-white dark:hover:bg-white dark:hover:text-zinc-900'
                      : 'border-border/80 bg-card/74 text-foreground hover:text-foreground'
                  }`}
                >
                  {cls.name}
                </button>
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(0,1fr)]">
          <section className="rounded-2xl border border-border/70 bg-card/58 p-4">
            ...
            <Slider
              ...
              className="py-3"
            />
            <div className="mt-2 grid gap-1 text-xs text-muted-foreground sm:flex sm:justify-between">
          ...
          <section className={`relative overflow-hidden rounded-2xl border p-4 ${statusSummaryPanelClass}`}>
            ...
            <div className="flex flex-col gap-3 pl-2 sm:flex-row sm:items-start sm:justify-between">
              ...
              <span
                className={`inline-flex shrink-0 items-center gap-1.5 self-start rounded-full border px-3 py-1 text-xs font-semibold ${statusBadgeClass}`}
              >
